...
use corinth::foreign_import::{build_foreign_recipe, parse_crux_pkgfile};
...
fn main() {
    if let Err(error) = run() {
        eprintln!("corinth-import-crux: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let pkgfile = read_regular(required(&flags, "pkgfile")?)?;
    ...
    let metadata = parse_crux_pkgfile(&pkgfile, &source_lock).map_err(|error| error.to_string())?;
...
fn usage() -> String {
    "usage: corinth-import-crux --pkgfile FILE --source-lock FILE --target FILE --target-signature FILE --keyring FILE --output ABSOLUTE_PATH".into()
}
